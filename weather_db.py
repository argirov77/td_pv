...
def extract_weather_from_new_db(user_object_id: int, prediction_date: str) -> list[dict]:
    """Extract weather from weather_data table in weather_main2 (new server)."""
    query = text(
        """
        SELECT current_data
        FROM weather_data
    ...
    try:
        ...
        raise WeatherArchiveError(
            "postgres_query",
            f"Ошибка чтения weather_data из weather_main2: {exc}",
            original_exception=exc,
        ) from exc
    ...
    df.sort_index(inplace=True)
    if df.index.has_duplicates:
        logger.warning("[extract_weather_from_new_db] Duplicate timestamps detected; aggregating before resample.")
        df = df.groupby(level=0).mean(numeric_only=True)
