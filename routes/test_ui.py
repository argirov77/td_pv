...
def require_admin(x_admin_token: str | None = Header(default=None)) -> None:
    if not ADMIN_TOKEN or x_admin_token != ADMIN_TOKEN:
        raise HTTPException(status_code=403, detail="Admin token is missing or invalid")
...
def admin_db_tables(_: None = Depends(require_admin)):
    query = text(
        """
        SELECT table_name
...
def admin_db_stats(_: None = Depends(require_admin)):
    query = text(
        """
...
def admin_cache_coverage(topic: str, date: str, _: None = Depends(require_admin)):
...
def admin_cache_select(request: CacheSelectRequest, _: None = Depends(require_admin)):
...
def admin_jobs_generate(request: GenerateRequest, _: None = Depends(require_admin)):
    generated = []
    errors = []
