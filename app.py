...
from forecast_db import (
    ...
    run_migrations,
    select_available_forecasts,
    select_points,
    select_topics_with_points,
)
...
def generate_cache(request: GenerateCacheRequest) -> GenerateCacheResponse:
    ...
    run_migrations()
    ensure_month_partitions(day_start, day_start + timedelta(days=1))
...
<body>
  ...
  <script>
    ...
    const getOptionalSelectedTopic = () => document.getElementById('topicSelect').value || "";
    ...
    const checkCoverage = async () => {
      const topic = getOptionalSelectedTopic();
