...
_LOGGED_MISSING_MODEL_PATHS = set()
_LOGGED_FALLBACK_SUCCESS_KEYS = set()
...
def load_model(tag: str):
    ...
    if not os.path.exists(model_path):
        ...
        if os.path.exists(fallback_path):
            if model_path not in _LOGGED_MISSING_MODEL_PATHS:
                logger.warning(
                    f"[load_model] Файлът с модел не е намерен: {model_path}. "
                    f"Използва се резервен модел: {fallback_path}"
                )
                _LOGGED_MISSING_MODEL_PATHS.add(model_path)
            try:
                logger.debug(f"[load_model] Опит за зареждане на резервен модел: {fallback_path}")
                model = joblib.load(fallback_path)
                fallback_key = (model_path, fallback_path)
                if fallback_key not in _LOGGED_FALLBACK_SUCCESS_KEYS:
                    logger.info(f"[load_model] Резервен модел зареден успешно от: {fallback_path}")
                    _LOGGED_FALLBACK_SUCCESS_KEYS.add(fallback_key)
                return model
                ...
                logger.error(
                    f"[load_model] Системна библиотека липсва при резервен модел: {ose}. "
                    "Проверете за libgomp.so.1 (пакет libgomp1) в контейнера."
                )
                return None
    ...
    try:
        ...
        logger.error(
            f"[load_model] Системна библиотека липсва при зареждане на модел: {ose}. "
            "Проверете за libgomp.so.1 (пакет libgomp1) в контейнера."
        )
        return None
    except Exception as e:
